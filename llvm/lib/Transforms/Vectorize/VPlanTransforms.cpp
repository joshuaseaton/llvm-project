...
void VPlanTransforms::multiversionForUnitStridedMemOps(
  ...
  DenseMap<VPValue *, VPValue *> ScalarizeMap;

  auto ScalarizePtrDefChainRecursively = [&](VPValue *PtrOp) {
    SmallVector<VPValue *, 8> Worklist;
    Worklist.push_back(PtrOp);
    while (!Worklist.empty()) {
      VPValue *CurrV = Worklist.back();
      if (ScalarizeMap.count(CurrV)) {
        Worklist.pop_back();
        continue;
      }
      auto *CurrI = dyn_cast<VPInstruction>(CurrV);
      if (!CurrI) {
        ScalarizeMap[CurrV] = CurrV;
        Worklist.pop_back();
        continue;
      }

      if (!RecipeBuilder.shouldWiden(CurrI->getUnderlyingInstr(), Range)) {
        // Legacy widening decided to scalarize this already, rely on it.
        ScalarizeMap[CurrV] = CurrV;
        Worklist.pop_back();
        continue;
      }

      // Legacy path wants to keep pointer computation on vectors (maybe because
      // there are other uses) but it's not clear if that's the best choice...

      bool OperandsReady = true;
      for (auto *Op : CurrI->operandsWithoutMask()) {
        if (ScalarizeMap.count(Op))
          continue;
      ...
      SmallVector<VPValue *> MappedOps;
      transform(CurrI->operandsWithoutMask(), std::back_inserter(MappedOps),
                [&](VPValue *V) -> VPValue * { return ScalarizeMap[V]; });

      auto Scalar = new VPReplicateRecipe(CurrI->getUnderlyingInstr(),
                                          MappedOps, true, CurrI->getMask(),
                                          *CurrI, *CurrI, CurrI->getDebugLoc());
      VPBuilder(CurrI).insert(Scalar);
      ScalarizeMap[CurrV] = Scalar;
    }
  };
  ...
  for_each(MemOps, [&](VPInstruction *VPI) {
    ...
    auto ReplaceWithUnitStrided = [&]() {
      ScalarizePtrDefChainRecursively(PtrOp);

      VPBuilder Builder(VPI);
      auto *VecPtr = new VPVectorPointerRecipe(ScalarizeMap[PtrOp], //
                                                                    // PtrOp, //
                                               ScalarTy, GEPNoWrapFlags::none(),
                                               VPI->getDebugLoc());
